...
import SaveItems from '@components/cart/BuildSaveCart/Saveitem.js';
...
export default function Cart() {
  ...
  useEffect(() => {
    let buildItemss: any= {}
    const temp_buildItems = data? data!.lineItems.filter((item: any, index:number) => item.customAttributes.length>0): [];
    const temp_cartItems = data? data!.lineItems.filter((item: any, index:number) => item.customAttributes.length==0): [];
    if(temp_buildItems.length > 0) {
      temp_buildItems.map((item:any, index:number) => {
        let key = item.customAttributes[0].value;
        if(buildItemss[key]) {
          buildItemss[key].push(item)
        } else {
          buildItemss[key] = [item]
        }
      })
      setBuildItems(buildItemss);
    }
            ...
            <ul className={s.lineItemsList}>
              {
                true && (
                  ...
                  Object.values(buildItems).map((item: any, index: number) => {
                    const build_token = item[0].customAttributes[0].value
                    const enquiry_token = item[0].customAttributes[1].value
                    const build_name = item[0].customAttributes[2].value
                    const build_image = item[0].customAttributes[3].value
                    const sub_total = item[0].customAttributes[4].value
                    return (
                      <SaveItems 
                        index={index}
                        key={index}
